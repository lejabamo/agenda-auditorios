...
export type AssistanceIntent = 'AVAILABILITY' | 'AGENDA' | 'GREETING' | 'UNKNOWN';

export interface AssistanceResponse {
    intent: AssistanceIntent;
    text: string;
    data?: any;
    displayType: 'text' | 'list' | 'matrix';
}

export const processQuery = (query: string, rawEvents: any[], auditorios: any[]): AssistanceResponse => {
    ...
    if (q.includes('libre') || q.includes('disponible') || q.includes('disponibilidad')) {
        // Simple strategy: check today or specific auditoriums mentioned
